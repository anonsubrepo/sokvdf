/****** Construction of Randomness Beacon using Pietrzak's Interactive Protocol *******/
...
const no = 30
for (let z = 0; z < no; z++){
    m[z] = bigInt.randBetween(0, N-1); // Randomly generated message 
}
// Connect to Client
...
function newConnection(socket){
    ...
    // Prepare to add delay to compute the output of the randomly generated messages
    // Send m, t, N and lamda to client who computes the VDF function
    io.emit('send_m', m);
    io.emit('send_t', t);
    io.emit('send_N', N);
    ...
    socket.on('send_x', (data) => {
        ...
        socket.on('send_y', (data) => {
            ...
            while (t_dash >= 1){
                t_dash1 = t_dash;
                r[i] = bigInt.randBetween(1, find_bit);
            ...
            socket.on('send_u', (data) => {
                ...
                if ((t_half >1) && (bigInt(t_half).isOdd())){
                    ...
                    y_dash[j+1] = bigInt(y_two[j]).mod(N);
                  }
                else if ((t_half >1)){
                ...
                if (bigInt(y_fin).equals(x_sqr)){
                    ...
                    console.log("The output of the beacon is", x);
                }else{
