/******************* Pietrzak's Non-Interactive Protocol*****************/
...
const lamda = 512; //RSA bit-length 
        ...
        socket.on('send_y', (data) => {
            ...
            socket.on('send_u', (data) => {
                ...
                if ((t_half >1) && (bigInt(t_half).isOdd())){
                    console.log("Odd first");
                    t_d = bigInt(t).add(1);
                    y_dash[0] = bigInt(y).modPow(2,N);
                    t_half = bigInt(t_d).divide(2);
                  }
                else {
                    console.log("Even first");
                    t_half = bigInt(t).divide(2);
                  }
                console.log("t_half=", t_half);
                var y_fin, x_fin;
                while (t_half >= 1){
                    r_one = bigInt(x_dash[j]).add(y_dash[j]);
                    r_two = bigInt(r_one).add(u[j]);
                    r_dash = bigInt(r_two).mod(N); 
                    var r_string = bigInt(r_dash).toString();
                    var r_hash = CryptoJS.SHA256(r_string)
                    r_hash.toString(CryptoJS.enc.Hex)
                    r[j] = bigInt(hextodec(r_hash)); // Since non-interactive, r = H(x'+y'+u)
